  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2 mb-6 text-sm text-gray-500">
        ...
        <span>/</span>
        <Link to={`/events/${id}`} className="hover:text-gray-700">{event?.name ?? '...'}</Link>
        <span>/</span>
        <span className="text-gray-900 font-medium">Lista de Convidados</span>
      </div>
      ...
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        ...
        <div className="flex gap-2 flex-wrap">
          <button onClick={() => setShowImport(true)}
            className="text-sm border border-gray-300 rounded-lg px-4 py-2 text-gray-700 hover:bg-gray-50 flex items-center gap-1.5">
      ...
      {showAddForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
          <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-md mx-4">
            <h3 className="font-bold text-gray-900 mb-4">Adicionar Convidado</h3>
      ...
      {showImport && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
          <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-lg mx-4">
            <h3 className="font-bold text-gray-900 mb-2">Importar do Google Sheets</h3>
            <p className="text-xs text-gray-500 mb-4">
              A planilha precisa ter colunas com os nomes: <strong>nome</strong>, <strong>telefone</strong>, <strong>email</strong>. Certifique-se de que ela está compartilhada como "qualquer pessoa com o link pode ver".
            </p>
            <label className="block text-xs font-medium text-gray-600 mb-1">URL da planilha</label>
            <input value={sheetUrl} onChange={e => setSheetUrl(e.target.value)}
              ...
              autoFocus />
            {importMsg && (
              <p className={`text-sm mb-3 ${importMsg.includes('sucesso') ? 'text-green-600' : 'text-red-500'}`}>{importMsg}</p>
            )}
            <div className="flex gap-2">
              <button onClick={() => { setShowImport(false); setImportMsg(''); setSheetUrl('') }}
                className="flex-1 border border-gray-300 rounded-lg py-2 text-sm text-gray-600 hover:bg-gray-50">Fechar</button>
      ...
      {guests.length === 0 ? (
        <div className="text-center py-16 text-gray-400">
          ...
          <p className="text-xs mt-1">Adicione manualmente ou importe do Google Sheets</p>
        </div>
      ) : (
