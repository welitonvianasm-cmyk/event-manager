    ...
    <div>
      ...
      <div className="flex gap-1 mb-6 bg-gray-800 p-1 rounded-xl w-fit">
        <button onClick={() => setTab('suppliers')}
          className={`px-4 py-2 text-sm font-medium rounded-lg transition-colors ${tab === 'suppliers' ? 'bg-gray-900 text-white shadow-sm' : 'text-gray-500 hover:text-gray-300'}`}>
          Fornecedores ({suppliers?.length ?? 0})
        </button>
        <button onClick={() => setTab('materials')}
          className={`px-4 py-2 text-sm font-medium rounded-lg transition-colors ${tab === 'materials' ? 'bg-gray-900 text-white shadow-sm' : 'text-gray-500 hover:text-gray-300'}`}>
      ...
      {showSupForm && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-gray-900 border border-gray-700 rounded-2xl p-6 w-full max-w-md">
            <h3 className="font-bold text-white mb-4">Cadastrar Fornecedor</h3>
            <div className="flex flex-col gap-3">
              <div><label className="block text-xs font-medium text-gray-400 mb-1">Nome *</label>
                <input value={supForm.name} onChange={e => setSupForm(f => ({ ...f, name: e.target.value }))}
                  className="w-full bg-gray-800 border border-gray-600 text-white rounded-lg px-3 py-2 text-sm" /></div>
              <div><label className="block text-xs font-medium text-gray-400 mb-1">Categoria</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-xs font-medium text-gray-400 mb-1">Telefone</label>
                  <input value={supForm.phone} onChange={e => setSupForm(f => ({ ...f, phone: e.target.value }))}
                    className="w-full bg-gray-800 border border-gray-600 text-white rounded-lg px-3 py-2 text-sm" /></div>
                <div><label className="block text-xs font-medium text-gray-400 mb-1">Nome do contato</label>
                  <input value={supForm.contactName} onChange={e => setSupForm(f => ({ ...f, contactName: e.target.value }))}
                    className="w-full bg-gray-800 border border-gray-600 text-white rounded-lg px-3 py-2 text-sm" /></div>
              </div>
              <div><label className="block text-xs font-medium text-gray-400 mb-1">E-mail</label>
                <input type="email" value={supForm.email} onChange={e => setSupForm(f => ({ ...f, email: e.target.value }))}
                  className="w-full bg-gray-800 border border-gray-600 text-white rounded-lg px-3 py-2 text-sm" /></div>
              <div><label className="block text-xs font-medium text-gray-400 mb-1">Endereço</label>
                <input value={supForm.address} onChange={e => setSupForm(f => ({ ...f, address: e.target.value }))}
                  placeholder="Rua, número, bairro, cidade"
                  className="w-full bg-gray-800 border border-gray-600 text-white placeholder-gray-600 rounded-lg px-3 py-2 text-sm" /></div>
              <div><label className="block text-xs font-medium text-gray-400 mb-1">Observações</label>
                <textarea value={supForm.notes} onChange={e => setSupForm(f => ({ ...f, notes: e.target.value }))} rows={2}
                  className="w-full bg-gray-800 border border-gray-600 text-white rounded-lg px-3 py-2 text-sm resize-none" /></div>
            </div>
            <div className="flex gap-3 mt-4">
              <button onClick={() => setShowSupForm(false)} className="flex-1 border border-gray-600 rounded-lg py-2 text-sm text-gray-400 hover:bg-gray-800">Cancelar</button>
      ...
      {showMatForm && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-gray-900 border border-gray-700 rounded-2xl p-6 w-full max-w-md">
            <h3 className="font-bold text-white mb-4">Cadastrar Material</h3>
            <div className="flex flex-col gap-3">
              <div><label className="block text-xs font-medium text-gray-400 mb-1">Nome *</label>
              ...
              <div><label className="block text-xs font-medium text-gray-400 mb-1">Categoria</label>
                <select value={matForm.category} onChange={e => setMatForm(f => ({ ...f, category: e.target.value }))}
                  className="w-full bg-gray-800 border border-gray-600 text-white rounded-lg px-3 py-2 text-sm">
                  {CATEGORIES.map(c => <option key={c} value={c}>{catLabel[c]}</option>)}</select></div>
              <div><label className="block text-xs font-medium text-gray-400 mb-1">Fornecedor preferido</label>
              ...
              <div><label className="block text-xs font-medium text-gray-400 mb-1">Preço padrão (R$)</label>
            ...
            <div className="flex gap-3 mt-4">
              <button onClick={() => setShowMatForm(false)} className="flex-1 border border-gray-600 rounded-lg py-2 text-sm text-gray-400 hover:bg-gray-800">Cancelar</button>
      ...
      {tab === 'suppliers' && (
        <div className="flex flex-col gap-3">
          {!suppliers?.length ? (
            <div className="text-center py-12 text-gray-600"><p className="text-3xl mb-2">🏪</p><p>Nenhum fornecedor cadastrado</p></div>
          ) : suppliers.map(s => (
            <div key={s.id} className="bg-gray-900 border border-gray-700 rounded-xl p-4 flex items-center justify-between hover:border-gray-600 transition-colors">
              <div>
                <div className="flex items-center gap-2">
                  <span className="font-medium text-white">{s.name}</span>
                  <span className="text-xs bg-gray-700 text-gray-400 px-2 py-0.5 rounded-full">{catLabel[s.category]}</span>
                </div>
                <p className="text-xs text-gray-500 mt-0.5">{[s.phone, s.contactName, s.email].filter(Boolean).join(' · ')}</p>
                {s.address && <p className="text-xs text-gray-500 mt-0.5">📍 {s.address}</p>}
              </div>
              <button onClick={() => { if (confirm('Excluir?')) deleteSupplier.mutate(s.id) }} className="text-xs text-red-500 hover:text-red-400">✕ Excluir</button>
            </div>
          ))}
        </div>
      )}
      ...
      {tab === 'materials' && (
        <div className="flex flex-col gap-3">
          {!materials?.length ? (
            <div className="text-center py-12 text-gray-600"><p className="text-3xl mb-2">📦</p><p>Nenhum material cadastrado</p></div>
          ) : materials.map(m => (
            <div key={m.id} className="bg-gray-900 border border-gray-700 rounded-xl p-4 flex items-center justify-between hover:border-gray-600 transition-colors">
              <div>
                <div className="flex items-center gap-2">
                  <span className="font-medium text-white">{m.name}</span>
                  <span className="text-xs bg-gray-700 text-gray-400 px-2 py-0.5 rounded-full">{catLabel[m.category]}</span>
                </div>
              ...
              <button onClick={() => { if (confirm('Excluir?')) deleteMaterial.mutate(m.id) }} className="text-xs text-red-500 hover:text-red-400">✕ Excluir</button>
            </div>
          ))}
        </div>
      )}
